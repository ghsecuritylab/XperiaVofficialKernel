...
static int check_android_listeners(fd_set *set) {
    if (FD_ISSET(STDIN_FILENO, set)) {
        ...
        /* Parse cmds and copy IPs to string in the format "1.2.3.4:1.2.3.4:..." */
        while (rc > 0) {
            char *next = bufptr;
            char *cmd;
            int cmdlen = strlen(bufptr);

            if ((cmd = strsep(&next, ":"))) {
                if (!strcmp(bufptr, "update_dns")) {
                    int serverslen = strlen(servers);
                    if (serverslen > 0 &&
                       (serverslen + 1) < sizeof(servers)) {
                        strncat(servers, ":", 1);
                        serverslen++;
                    }
                    if ((serverslen + cmdlen - 11) < sizeof(servers)) {
                        strncat(servers, bufptr + 11, cmdlen - 11);
                    } else {
                        my_syslog(LOG_ERR, _("servers length overflow 1024"));
                    }
                } else
                    my_syslog(LOG_ERR, _("Unknown cmd '%s'"), cmd);
            } else
                my_syslog(LOG_ERR, _("Malformatted msg '%s'"), bufptr);
        ...
        if (strlen(servers)) {
            ...
            my_syslog(LOG_ERR, _("No valid servers."));
            return -1;
        }
    }
    return 0;
