...
static void check_if_skip(e2fsck_t ctx)
{
	...
	if (ctx->start_time.tv_nsec)  {
		struct timespec now;
		clock_gettime( CLOCK_MONOTONIC, &now);
		printf("%s Total check time:%lld us code:%d\n",
		       LOGTAG,CLOCK_DIFF(now,ctx->start_time),FSCK_OK);
	}
	exit(FSCK_OK);
}
...
static errcode_t PRS(int argc, char *argv[], e2fsck_t *ret_ctx)
{
	...
	*ret_ctx = ctx;
	...
	while ((c = getopt (argc, argv, "panyrcC:B:dE:fvtFVM:b:I:j:P:l:L:N:SsDko:T")) != EOF)
		switch (c) {
		case 'C':
			ctx->progress = e2fsck_update_progress;
	...
	_("Only one of the options -p/-a, -n or -y may be specified."));
		...
		case 'o':  {
				int sdfd;
				ctx->logname = string_copy(ctx, optarg, 0);
				sdfd = open(ctx->logname,O_RDWR | O_CREAT | O_APPEND,0666);
				if (sdfd != -1)  {
					dup2(sdfd,STDOUT_FILENO);
					dup2(sdfd,STDERR_FILENO);
                                        close(sdfd);
				} else {
					fprintf(stderr,"Output file failed %s [%d]\n",ctx->logname,errno);
				}
			}
			break;
		case 'T':
...
no_journal:
	...
	if (ctx->start_time.tv_nsec)  {
		...
		clock_gettime( CLOCK_MONOTONIC, &now);
		printf("%s: Total check time:%lld us code:%d\n",
		       LOGTAG,CLOCK_DIFF(now,ctx->start_time),exit_value);
	}
	e2fsck_free_context(ctx);
